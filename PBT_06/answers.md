...
1. **Công cụ:** Node.js, npm, **Sass compiler** (dart-sass), tùy chọn bundler (Vite/Webpack) hoặc CLI Bootstrap.
2. **Tạo project custom:**
   ...
   - Tạo file SCSS riêng, ví dụ `scss/custom.scss`:
     ...
     ```
3. **Biên dịch** SCSS → CSS (`sass scss/custom.scss dist/css/bootstrap-custom.css`).
4. **HTML** link file CSS đã build thay vì CDN Bootstrap mặc định.
5. **Tùy chọn:** dùng `_variables.scss` override trước khi `@import` Bootstrap, hoặc `bootstrap.scss` chỉ import các module cần để giảm dung lượng.
...
| File | Mô tả |
|------|--------|
| `bootstrap_landing.html` | Landing E-Commerce (B1) |
| `bootstrap_dashboard.html` | Admin Dashboard (B2) |
