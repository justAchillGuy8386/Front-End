...
1. **`var` hoisting:** `var x` được đưa lên đầu scope; `console.log(x)` chạy khi `x` đã được khai báo nhưng chưa gán → `undefined`.
2. **`let` TDZ:** `let y` không hoist như `var` để dùng trước dòng khai báo → **ReferenceError**.
3. **`const`:** Cấm **gán lại** binding (`z = 20`), không cấm **đột biến** nội dung object/array (`arr.push`).
4. **Block scope:** `let a` trong `{}` là biến riêng, không ảnh hưởng `a` ngoài block.
...
| File | Mô tả |
|------|--------|
| `var_let_const.js` | Kiểm chứng A1 |
| `calculator.js` | Máy tính B1 |
| `student_data.js` | Dữ liệu sinh viên B2 |
| `guess_number.html` + `guess.js` | Game đoán số B3 (mở HTML trong trình duyệt) |
| `fizzbuzz.js` | FizzBuzz classic + custom B4 |
| `restaurant_bill.js` | Hóa đơn nhà hàng C2 |
...
Mở `guess_number.html` trong Chrome để chơi game (dùng `prompt` / `alert`).

Chụp màn hình kết quả console vào folder `screenshots/` khi nộp bài.
